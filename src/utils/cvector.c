...
# define CWARNING_ITER(cv, iter, file, func, line) \
	...
	paradise_debug("var(" #iter ") warng out of range, "\
...
void *vmalloc_realloc(void *old_ptr, size_t old_size, size_t new_size) {
    ...
    if (!new_ptr) {
        paradise_debug("vmalloc realloc failed for size: %zu\n", new_size);
        return NULL;
    }
...
void cv_info(const cvector cv)
{
	...
	paradise_debug("len :%s : %zu\n",     __func__, cv->cv_len);
	paradise_debug("size:%s : %zu\n\n",   __func__, cv->cv_size);
	return;
}
...
void cv_print(const cvector cv)
{
	...
	if (cvector_length(cv) == 0)
		paradise_debug("file:%s func:%s line:%d error, null length cvector!!\n", __FILE__, __func__, __LINE__);
